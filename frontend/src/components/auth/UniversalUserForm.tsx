...
const UniversalUserForm = ({ type, initialData, onSuccess, role }: Props) => {
  ...
  // Fetch related parents / students for synchronization mapping
  useEffect(() => {
    if (type !== "login") {
      const fetchRelatedUsers = async () => {
        try {
          if (role === "student") {
            ...
            setAllParents(data.users || data || []);
          } else if (role === "parent") {
            const { data } = await api.get("/users?role=student");
            setAllStudents(data.users || data || []);
          }
        } catch (e) {
          console.error("Failed to load parent/student list for sync", e);
        }
      };
      fetchRelatedUsers();
    }
  ...
  useEffect(() => {
    if (initialData && isUpdate) {
      ...
      const parentId = typeof initialData.parent === "object"
        ? (initialData.parent as any)?._id
        : initialData.parent;

      const childrenIds = initialData.parentOf?.map((child: any) => 
  ...
  async function onSubmit(data: FormValues) {
    try {
      // console.log(data);
      const { confirmPassword, classId, subjectIds, parent, parentOf, ...restData } = data;
      const payload = {
      ...
      <FieldGroup>
        <div className="grid grid-cols-2 gap-4 w-full">
          ...
          <div className="col-span-2 space-y-2">
            ...
            {/* parent (student creation parent sync) */}
            {role === "student" && !isLogin && (
              <CustomSelect
                control={form.control}
                name="parent"
                label="Parent Link (Synchronization)"
                placeholder="Select Parent for synchronization"
            ...
            {/* children (parent creation children sync) */}
            {role === "parent" && !isLogin && (
              <CustomMultiSelect
                control={form.control}
                name="parentOf"
                label="Student Link (Sync Children)"
                placeholder="Select children to map..."
